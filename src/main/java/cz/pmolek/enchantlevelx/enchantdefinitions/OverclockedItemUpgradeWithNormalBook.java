...
public class OverclockedItemUpgradeWithNormalBook implements AnvilEnchantDefinition {
    ...
    public boolean validateInput(@Nullable ItemStack left, @Nullable ItemStack right, @Nullable ItemStack result) {
        return result != null
                ...
                && !EnchantmentUtils.getEnchantmentsThat(left, (ench, level) -> level > ench.getMaxLevel()).isEmpty();
    }

    @Override
    public ItemStack enchant(ItemStack left, ItemStack right, PrepareAnvilEvent eventData) {
        ItemStack result = eventData.getResult();
        for (Tuple<Enchantment, Integer> entry : EnchantmentUtils.getEnchantmentsThat(left, (ench, level) -> level > ench.getMaxLevel())) {
            EnchantmentUtils.addEnchantmentUnsafe(result, entry.getA(), entry.getB());
        }
        return result;
    }

...
public abstract class CharmEnchantBase implements AnvilEnchantDefinition {
    ...
    {
        Tuple<Enchantment, Integer> enchantment = EnchantmentUtils.getEnchantmentsThat(output, this::isValidEnchantment).get(0);
        EnchantmentUtils.addEnchantmentUnsafe(output, enchantment.getA(), enchantment.getB() + 1);
    }
    ...
    protected ItemMeta createLore(ItemMeta meta, CharmModelData charmData)
    {
        ...
        lore.clear();
        lore.add(ChatColor.WHITE + "Charmed with " + charmData.color + "Charm of Enchanting " + "I".repeat(charmData.level) + ChatColor.WHITE + ".");
        meta.setLore(lore);
        return meta;
    }

    protected void applyItemMeta(ItemStack output, CharmModelData charmData, AnvilInventory eventData)
    {
